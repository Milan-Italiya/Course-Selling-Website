    ...
    useEffect(() => {
        if (!token) {
            ...
            setTimeout(() => {
                navigate("/login", {
                    state: {
                        loginerrMessage: "Please login to buy the course",
                    },
                });
            }, 800);
        }
            ...
            try {
                ...
                if (data?.status === 400 || data?.success === false) {
                    navigate("/purchases", {
                        state: {
                            purchaseError:
                                data?.errors || "You have already purchased this course",
                        },
                    });
        ...
        try {
            ...
            if (confirmError) {
                ...
                if (response?.ok) {
                    // navigate("/purchases", {
                    //     state: {
                    //         purchaseMessage: "Payment successful. Waiting for admin approval.",
                    //     },
                    // });
