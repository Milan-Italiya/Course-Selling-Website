...
import { topCourseUsecase } from "../Usecases/TopCourseUsecase.jsx";
  ...
  export const CourseProvider = ({ children }) => {
    ...
    const fetchTopCourses = async (topCourseData) => {
      try {
        const response = await topCourseUsecase(topCourseData);
        console.log("Top Courses: ",response)
        setTopCourses(response)
      } catch (error) {
        console.log("Failed to fetch top courses: ",error);
      }
    }
    return (
      <CourseContext.Provider value={{ course, courseDetails, order,topCourses, fetchCourses, fetchCourseDetails, fetchCoursesBySearch, fetchBuyCourse, fetchOrderCourse,fetchTopCourses }}>
        {children}
