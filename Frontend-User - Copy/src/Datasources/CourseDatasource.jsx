...
export const topCourse = async (topCourseData) => {
  try {
    const response = await fetch(`http://localhost:5000/api/v1/course/top-courses`)
    const data = await response.json(topCourseData)
    console.log("top courses datasourse: ",data.PopularCourses)
    return data.PopularCourses
  } catch (error) {
    console.log("Error in top courses: ",error)
  }
}